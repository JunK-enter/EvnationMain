...
const HERO_SERVICES = [
  ...
  { label: 'Water Heaters', icon: Droplets, href: '/quote', hideMobile: true },
  { label: 'Dryers & Ovens', icon: Wind, href: '/quote', hideMobile: true },
]
...
export default function Hero() {
  return (
    <section className="relative lg:min-h-screen flex items-center overflow-hidden pt-[max(4.25rem,env(safe-area-inset-top))] lg:pt-16">
      <div className="absolute inset-x-0 bottom-0 h-24 bg-gradient-to-t from-navy-950/80 to-transparent pointer-events-none" />

      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-16 lg:py-24 w-full">
        <div className="flex flex-col lg:grid lg:grid-cols-[1.05fr_0.95fr] gap-6 lg:gap-14 xl:gap-20 lg:items-center">
          ...
          >
            ...
            >
              <span className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full glass-light text-xs text-neon border border-neon/15">
                <span className="relative flex h-2 w-2 shrink-0">
                  <span className="max-lg:animate-ping absolute inline-flex h-full w-full rounded-full bg-neon opacity-40" />
            ...
            <p className="text-sm sm:text-base text-slate-400 leading-relaxed mb-6 sm:mb-8 w-full max-w-md lg:max-w-none mx-auto lg:mx-0">
              Level 2 charging, panel upgrades, Powerwall & solar — one licensed team across {serviceArea.regionShort}.
            </p>

            {/* CTAs before chips — visible above the fold */}
            <div className="flex flex-col sm:flex-row gap-3 w-full sm:w-auto mb-6 sm:mb-8">
              <Link href="/quote" className="btn-primary w-full sm:w-auto justify-center !py-3.5 !px-8 !text-base min-h-[48px] shadow-[0_0_40px_rgba(0,255,136,0.2)]">
                Build Your Quote
                <ArrowRight className="w-4 h-4" />
              </Link>
              <Link href="/intake" className="btn-secondary w-full sm:w-auto justify-center !py-3.5 !px-8 min-h-[48px]">
              ...
              {HERO_SERVICES.map((item, i) => {
                ...
                return (
                  ...
                  >
                    <Link
                      href={item.href}
                      className="group flex sm:inline-flex items-center gap-2 w-full sm:w-auto px-3 py-2.5 sm:px-3.5 rounded-xl sm:rounded-full text-xs font-medium text-slate-300 bg-white/[0.04] border border-white/10 hover:border-neon/40 hover:bg-neon/[0.07] hover:text-white active:scale-[0.98] transition-all duration-300"
                    >
          ...
          {/* Visual — below copy on mobile, beside on desktop */}
          <motion.div
            initial={{ opacity: 0, scale: 0.96, y: 12 }}
            animate={{ opacity: 1, scale: 1, y: 0 }}
            transition={{ duration: 0.85, delay: 0.2, ease: [0.22, 1, 0.36, 1] }}
            className="relative w-full max-w-sm mx-auto lg:max-w-none order-2 hidden sm:block"
          >
            <div className="absolute -inset-4 rounded-[2rem] bg-gradient-to-br from-neon/15 via-transparent to-blue-500/10 blur-2xl opacity-60 pointer-events-none" />
            <HeroVisual />
          </motion.div>
        </div>
      </div>
