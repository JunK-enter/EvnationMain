...
export default function ElectricianPortalPage() {
  const { user, login, logout } = useAuth()
  const [email, setEmail] = useState('electrician@evnation.com')
  ...
  function handleLogin(e) {
    ...
    else if (result.user?.role !== 'electrician') {
      setError('Electrician credentials required')
      logout()
    }
  }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center px-4">
        <div className="glass rounded-2xl p-8 w-full max-w-md neon-border">
          ...
          <p className="text-center text-sm text-slate-400 mb-6">Electrician Portal</p>
          ...
          <p className="text-xs text-slate-500 text-center mt-4">Demo: electrician@evnation.com / demo</p>
    ...
    <>
      <div className="fixed top-0 left-0 right-0 z-50 glass border-b border-white/5 h-14 flex items-center justify-between px-6">
        <Link to="/" className="flex items-center gap-2 hover:opacity-90 transition-opacity">
          ...
          <span className="font-display font-bold text-sm">Electrician</span>
        </Link>
