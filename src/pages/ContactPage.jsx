...
export default function ContactPage() {
  ...
  async function handleSubmit(e) {
    e.preventDefault()
    const quote = calculateQuote({ state: form.state || 'CA', distance: Number(form.distance) || 25 })

    const submission = await createSubmission({
      personal: { firstName: form.firstName, lastName: form.lastName, email: form.email, phone: form.phone },
      home: { state: form.state, city: form.city, zip: form.zip, homeType: form.homeType, panelSize: form.panelSize, parking: form.parking, distance: form.distance },
